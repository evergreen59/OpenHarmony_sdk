...
#define W25Q_QuadRead              0x6B
#define W25Q_QuadWrite             0x32 
...
static void W25QxxWriteEnable_QSPI(void)
{
    uint8_t wbuf[3] = {W25Q_WriteStatusReg, 0x00, 0x02};
    uint8_t rbuf[3] = {0};
    struct SpiMsg msg = {0};
    msg.wbuf = wbuf;
    msg.rbuf = rbuf;
...
static uint8_t W25QxxReadStatusReg(uint16_t delay) {
...
static void W25QxxWaitForWriteEnd(void)
{
    ...
    do {
        status = W25QxxReadStatusReg(BUSY_DETECT_DELAY);
#if DEBUG_SPI_FLASH
...
static void W25QxxWritePage(uint8_t* pBuffer, uint32_t WriteAddr, uint32_t NumByteToWrite)
{
    ...
    uint8_t *rbuf1 = (uint8_t*)OsalMemAlloc(NumByteToWrite);
    if (rbuf1 == NULL) {
 ...
 */
HDF_STATUS W25Qxx_InitQspiFlash(uint32_t busNum, uint32_t csNum)
{
 ...
 * deinit spi
 */
HDF_STATUS W25Qxx_DeInitQspiFlash(void)
{
    if (spiHandle != NULL) {
        SpiClose(spiHandle);
        HDF_LOGI("%s: success(%x)", __func__, spiHandle);
 ...
 */
void W25Qxx_QSPI_EraseSector(uint32_t SectorAddr)
{
...
*/
void W25Qxx_QSPI_EraseWholeChip(void)
{
    CHECK_SPI_HANDLE(spiHandle);
 ...
 */
void W25Qxx_QSPI_Write_NoCheck(uint8_t* pBuffer, uint32_t WriteAddr, uint32_t NumByteToWrite)
{
    CHECK_SPI_HANDLE(spiHandle);
...
void W25Qxx_QSPI_Read(uint8_t* pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead)
{
    CHECK_SPI_HANDLE(spiHandle);
...
*/
uint32_t W25Qxx_QSPI_ReadJedecID(void)
{
    if (spiHandle == NULL) {
        HDF_LOGE("%s: spi handler hasn't been inited", __func__);
        return 0;
...
void W25Qxx_Qspi_PowerDown(void)
{
    CHECK_SPI_HANDLE(spiHandle);
