...
void crm_flag_clear(uint32_t flag)
{
  switch(flag)
  {
    ...
    case CRM_ALL_RESET_FLAG:
      CRM->ctrlsts_bit.rstfc = AT_TRUE;
      break;
    case CRM_LICK_READY_INT_FLAG:
      CRM->clkint_bit.lickstblfc = AT_TRUE;
      break;
    case CRM_LEXT_READY_INT_FLAG:
      CRM->clkint_bit.lextstblfc = AT_TRUE;
      break;
    case CRM_HICK_READY_INT_FLAG:
      CRM->clkint_bit.hickstblfc = AT_TRUE;
      break;
    case CRM_HEXT_READY_INT_FLAG:
      CRM->clkint_bit.hextstblfc = AT_TRUE;
      break;
    case CRM_PLL_READY_INT_FLAG:
      CRM->clkint_bit.pllstblfc = AT_TRUE;
      break;
    case CRM_CLOCK_FAILURE_INT_FLAG:
      CRM->clkint_bit.cfdfc = AT_TRUE;
      break;
    default:
      break;
  }
...
void crm_pll_config(crm_pll_clock_source_type clock_source, crm_pll_mult_type mult_value, crm_pll_output_range_type pll_range)
{
  ...
  {
    CRM->cfg_bit.pllrcs = AT_FALSE;
  }
  else
  {
    CRM->cfg_bit.pllrcs = AT_TRUE;
    if(CRM_PLL_SOURCE_HEXT == clock_source)
    {
      CRM->cfg_bit.pllhextdiv = AT_FALSE;
    }
    else
    {
      CRM->cfg_bit.pllhextdiv = AT_TRUE;
    }
  }
