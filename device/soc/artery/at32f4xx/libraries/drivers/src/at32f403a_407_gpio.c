...
void gpio_reset(gpio_type *gpio_x)
{
  if(gpio_x == GPIOA)
  {
    crm_periph_reset(CRM_GPIOA_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_GPIOA_PERIPH_RESET, AT_FALSE);
  }
  else if(gpio_x == GPIOB)
  {
    crm_periph_reset(CRM_GPIOB_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_GPIOB_PERIPH_RESET, AT_FALSE);
  }
  else if(gpio_x == GPIOC)
  {
    crm_periph_reset(CRM_GPIOC_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_GPIOC_PERIPH_RESET, AT_FALSE);
  }
  else if(gpio_x == GPIOD)
  {
    crm_periph_reset(CRM_GPIOD_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_GPIOD_PERIPH_RESET, AT_FALSE);
  }
  else if(gpio_x == GPIOE)
  {
    crm_periph_reset(CRM_GPIOE_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_GPIOE_PERIPH_RESET, AT_FALSE);
  }
}
...
{
  crm_periph_reset(CRM_IOMUX_PERIPH_RESET, AT_TRUE);
  crm_periph_reset(CRM_IOMUX_PERIPH_RESET, AT_FALSE);
}
...
/**
  ...
  *         - GPIO_PINS_15
  *         - GPIO_PINS_ALL
  * @param  bit_state: specifies the value to be written to the selected bit (TRUE or AT_FALSE).
  * @retval none
  */
void gpio_bits_write(gpio_type *gpio_x, uint16_t pins, confirm_state bit_state)
{
  if(bit_state != AT_FALSE)
  {
    gpio_x->scr = pins;
  }
  else
...
/**
  ...
  *         this parameter can be: TRUE or AT_FALSE.
  * @retval none
  */
...
/**
  ...
  *         - UART8_GMUX
  * @param  new_state: (TRUE or AT_FALSE)
  * @retval none
  */
