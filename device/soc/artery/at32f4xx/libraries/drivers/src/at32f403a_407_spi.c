...
{  
  if(spi_x == SPI1)
  {
    crm_periph_reset(CRM_SPI1_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_SPI1_PERIPH_RESET, AT_FALSE);
  }
  else if(spi_x == SPI2)
  {
    crm_periph_reset(CRM_SPI2_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_SPI2_PERIPH_RESET, AT_FALSE);
  } 
  else if(spi_x == SPI3)
  {
    crm_periph_reset(CRM_SPI3_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_SPI3_PERIPH_RESET, AT_FALSE);
  } 
  else if(spi_x == SPI4)
  {
    crm_periph_reset(CRM_SPI4_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_SPI4_PERIPH_RESET, AT_FALSE);
  }   
}
...
void spi_init(spi_type* spi_x, spi_init_type* spi_init_struct)
{
  ...
  {
    spi_x->ctrl1_bit.swcsil = AT_TRUE;  
  }
  else
  {
    spi_x->ctrl1_bit.swcsil = AT_FALSE; 
  }
...
void i2s_default_para_init(i2s_init_type* i2s_init_struct)
{
  ...
  i2s_init_struct->mclk_output_enable = AT_FALSE;     
}
...
void i2s_init(spi_type* spi_x, i2s_init_type* i2s_init_struct)
{
  ...
  else
  {
    ...
    {
      if(i2s_init_struct->mclk_output_enable == AT_TRUE)
      {
  ...
  spi_x->i2sctrl_bit.i2smsel = AT_TRUE;
}
