...
void usart_reset(usart_type* usart_x)
{
  if(usart_x == USART1)
  {
    crm_periph_reset(CRM_USART1_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_USART1_PERIPH_RESET, AT_FALSE);
  }
  else if(usart_x == USART2)
  {
    crm_periph_reset(CRM_USART2_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_USART2_PERIPH_RESET, AT_FALSE);
  }
  else if(usart_x == USART3)
  {
    crm_periph_reset(CRM_USART3_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_USART3_PERIPH_RESET, AT_FALSE);
  }
  else if(usart_x == UART4)
  {
    crm_periph_reset(CRM_UART4_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_UART4_PERIPH_RESET, AT_FALSE);
  }
  else if(usart_x == UART5)
  {
    crm_periph_reset(CRM_UART5_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_UART5_PERIPH_RESET, AT_FALSE);
  }
  else if(usart_x == USART6)
  {
    crm_periph_reset(CRM_USART6_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_USART6_PERIPH_RESET, AT_FALSE);
  }
  else if(usart_x == UART7)
  {
    crm_periph_reset(CRM_UART7_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_UART7_PERIPH_RESET, AT_FALSE);
  }
  else if(usart_x == UART8)
  {
    crm_periph_reset(CRM_UART8_PERIPH_RESET, AT_TRUE);
    crm_periph_reset(CRM_UART8_PERIPH_RESET, AT_FALSE);
  }
}
...
void usart_parity_selection_config(usart_type* usart_x, usart_parity_selection_type parity)
{
  ...
  {
    usart_x->ctrl1_bit.psel = AT_FALSE;
    usart_x->ctrl1_bit.pen = AT_FALSE;   
  }
  else if(parity == USART_PARITY_EVEN)
  {
    usart_x->ctrl1_bit.psel = AT_FALSE;
    usart_x->ctrl1_bit.pen = AT_TRUE;   
  }
  else if(parity == USART_PARITY_ODD)
  {
    usart_x->ctrl1_bit.psel = AT_TRUE;
    usart_x->ctrl1_bit.pen = AT_TRUE;   
  }
}
...
/**
  ...
  * @retval none
  */
void usart_transmit_str(usart_type* usart_x, char *str)
{
    int i = 0;
    do {
        usart_data_transmit(usart_x, *(str + i));
        delay_ms(1);
        i++;
    }while(*(str + i) != '\0');
}
...
void usart_hardware_flow_control_set(usart_type* usart_x,usart_hardware_flow_control_type flow_state)
{
  ...
  {
    usart_x->ctrl3_bit.rtsen = AT_FALSE;
    usart_x->ctrl3_bit.ctsen = AT_FALSE;
  }
  else if(flow_state == USART_HARDWARE_FLOW_RTS)
  {
    usart_x->ctrl3_bit.rtsen = AT_TRUE;
    usart_x->ctrl3_bit.ctsen = AT_FALSE;
  }
  else if(flow_state == USART_HARDWARE_FLOW_CTS)
  {
    usart_x->ctrl3_bit.rtsen = AT_FALSE;
    usart_x->ctrl3_bit.ctsen = AT_TRUE;
  }
