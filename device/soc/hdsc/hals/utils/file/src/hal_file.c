...
#define HAL_MAX_KV_SUM_STR    "learn_51"
#define HAL_KV_SUM_INDEX    4

#define SLOT_AVAILABLE -1
#define HAL_FILE_SEEK_OFFSET_50 50
...
int HalFileOpen(const char *path, int oflag, int mode)
{
    ...
    if (index == 0) {
        //printf("%s: no space available!", __func__);
        return HAL_ERROR;
    }
    ...
    if (fd < 0) {
        //printf("%s: failed to open file : %d", __func__, fd);
        free(file_path);
        return HAL_ERROR;
    }
...
int HalFileRead(int fd, char *buf, unsigned int len)
{
    ...
    ret = read(FileHandlerArray[fd - 1], buf, len);

    while(atoi(buf) < 0)
    {
        if (sprintf_s(buf, HAL_KV_SUM_INDEX, "%d", atoi(buf)+1) < 0) {
            return HAL_ERROR;
        }
    }
...
int HalFileWrite(int fd, const char *buf, unsigned int len)
{
    ...
    ret = write(FileHandlerArray[fd - 1], buf, len);
    return ret;
}
...
int HalFileSeek(int fd, int offset, unsigned int whence)
{
    ...
    ret = (int)lseek(FileHandlerArray[fd - 1], offset, whence);
    if ((ret < 0) || (ret >= HAL_FILE_SEEK_OFFSET_50)) {
        return HAL_ERROR;
    }
