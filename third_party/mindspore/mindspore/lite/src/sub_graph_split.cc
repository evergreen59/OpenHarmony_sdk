...
void SearchSubGraph::UpdateOfflineParallelFlag() {
  ...
  offline_parallel_enable_ = std::any_of(
    ...
    [&](lite::LiteGraph::Node *node) { return IsOfflineParallelNode(node->primitive_, node->device_type_); });
}
...
SearchSubGraph::CostModel SearchSubGraph::CalculateConv2DFusion(LiteGraph::Node *node) {
  CostModel cost;
...
void SearchSubGraph::ConvertSubGraphToModel(std::vector<Subgraph> *sub_graphs) {
  ...
  for (Subgraph &subgraph : *sub_graphs) {
    ...
    new_partial_node->node_type_ = static_cast<int>(mindspore::lite::MSNodeType::NodeType_ValueNode);
...
void SearchSubGraph::InitSearchTensor() {
  ...
  for (size_t i = 0; i < tensors_.size(); i++) {
    ...
    mindspore::schema::Tensor *src_tensor = static_cast<schema::Tensor *>(model_->graph_.all_tensors_[i]);
