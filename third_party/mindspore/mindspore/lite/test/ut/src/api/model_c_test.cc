...
namespace mindspore {
class ModelCTest : public mindspore::CommonTest {
 public:
  ModelCTest() {}
  int GenerateInputDataWithRandom(OH_AI_TensorHandleArray inputs);
};

int ModelCTest::GenerateInputDataWithRandom(OH_AI_TensorHandleArray inputs) {
...
TEST_F(ModelCTest, build) {
  ...
  size_t size = 0;
  auto buff = lite::ReadFile("../ut/test_data/mobilenet_v1_1.0_224.ms", &size);
...
TEST_F(ModelCTest, predict) {
  ...
  // Build model
  int ret = OH_AI_ModelBuildFromFile(model, "../ut/test_data/mobilenet_v1_1.0_224.ms", OH_AI_MODELTYPE_MINDIR, context);
